...
* 
*  It either generates home page from cmsTemplate file (inits main menu, header, footer)
    ...
    if(!isEmptyArray($website_languages)){
        ...
        foreach($website_languages as $term_id){
            ...
            if($lang_code){
                ...
                $res_opts = $res_opts.'<option value="'.$lang_code.'">'.$lang_code.'</option>';
                array_push($website_languages_res, $lang_code);
            }
        }
        $website_languages_links = count($website_languages_res)>1?$res:'';
        $website_languages_options = count($website_languages_res)>1?$res_opts:'';
...
if(!empty($website_languages_links)){
    ...
    $curr_lang_text = 'value="'.$website_language_def.'"';
    $website_languages_options = str_replace($curr_lang_text, $curr_lang_text . ' selected', $website_languages_options);
}
...
function stripOuterFooterTag($html) {
    $html = trim($html);

    if (preg_match('/^<footer\b[^>]*>(.*)<\/footer>$/is', $html, $matches)) {
        return trim($matches[1]);
    }

    return $html;
}
function containsMainHost($html) {
    return preg_match('/\bid\s*=\s*["\']main-host["\']/i', $html) === 1;
}
...
if(!$isWebPage){  //not standalone web page
    ...
    //== ConceptCode::getTermLocalID('2-532') || $page_footer_type == ConceptCode::getTermLocalID('2-531'))
    {
        ...
        $page_footers = null;
        if(defined('DT_CMS_FOOTER')){ //custom footer
        ...
        if (@$site_owner['ugr_eMail']) {
            $mailto = '<div class="footer-item">&bull;&nbsp;&nbsp;<a href="mailto:'.@$site_owner['ugr_eMail'].'" target="_blank">site owner</a></div>';
        }        
        ...
        <div id="main-host">
            ...
            <div class="footer-item">
                ...
                title="This website is generated by Heurist, an academic knowledge management system developed under the direction of Dr Ian Johnson, chief programmer Artem Osmakov">
                powered by <img src="$favicon_url" style="vertical-align:sub"> Heurist
                </a>
            </div>
            <div id="host_info" class="footer-item"></div>
        </div>
        EXP;
        ...
        $custom_footer_has_main_host = false;

        if ($page_footers != null) {
            foreach ($page_footers as $val) {
                list($lang, $val) = extractLangPrefix($val);

                $val = stripOuterFooterTag($val);

                if (containsMainHost($val)) {
                    $custom_footer_has_main_host = true;
                }

                $st = (
                    count($page_footers) == 1 ||
                    ($current_language == $lang) ||
                    ($current_language == $website_language_def && $lang == null)
                ) ? '' : ' style="display:none"';

                $page_footer .=
                    '<div class="page-footer-content footer-flex" data-lang="'.($lang != null ? $lang : '').'"'.$st.'>'
                    .$val
                    .DIV_E;
            }
        }

        if (!$custom_footer_has_main_host) {
            $page_footer .= $host_information;
        }
...
if($template!==false){
    ...
    if(!file_exists($template)){
        ...
        include_once HEURIST_DIR.'hclient/widgets/cms/cmsTemplate.php';
    }
}

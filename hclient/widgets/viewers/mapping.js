...
* 4) thematic mapping
* 5) map document  - get lists, get content by id (as geojson), load layers (as FeatureCollection)
...
$.widget( "heurist.mapping", {
    ...
    main_layer: null, //curren search layer
    all_top_layers: [], //aray of all loaded geojson layers
    main_popup:null,
    selected_rec_ids:[],
...
*/
    ...
    */
    ...
    resetStyle: function(layer) {
      ...
      if(!layer) layer = this.main_layer;
    ...
    addDataset: function(data, dataset_name) 
    {
        ...
        if(curr_request!=null){
            ...
            window.hWin.HAPI4.RecordMgr.search_new(curr_request,
                function(response){
                    ...
                    if(window.hWin.HEURIST4.util.isGeoJSON(response)){
        ...
        }else if (window.hWin.HEURIST4.util.isGeoJSON(data)){
...
//console.log('onclick');                
    ...
    setVisibility: function( _selection, is_visible ){
        ...
        if(_selection===true || window.hWin.HEURIST4.util.isArrayNotEmpty(_selection)) {
            ...
            this.main_layer.eachLayer(function(layer){
                  if (_selection===true || _selection.indexOf(layer.feature.properties.rec_ID)>=0) 
                  {
                      if(is_visible==false){
                            layer.remove();    

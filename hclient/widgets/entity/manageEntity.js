...
$.widget( "heurist.manageEntity", {
    ...
    _initControls:function(){
       ...
       if(this.options.edit_mode!='none'){
            //editor container
            this.ent_editor_wrapper = $('<div>',{'id':'editor_wrapper'}).appendTo(this.wrapper);
            //edit form
            this.ent_editor_header = $('<div>')
                                .addClass('ent_header')
                                .css({'padding':'0.2em 0.5em'})
                                .appendTo(this.ent_editor_wrapper);
            this.ent_editor = $('<div>',{'id':'editor_panel'})
                                .addClass('ent_content_full')
                                .html('<div class="center-message">Select an entity in the list to edit</div>')
                                .appendTo(this.ent_editor_wrapper);
...
//      .css('min-width', this.options.edit_width?this.options.edit_width:'700px')
            if(this.options.edit_mode=='inline'){
               ...
               if(this.recordList){
                   ...
                   this.ent_editor.css({'border-top':'1px solid #cccccc',
    ...
    */
    ...
    _destroy: function() {
        ...
        if(this.searchRecord) this.searchRecord.remove();
        if(this.recordList) this.recordList.remove();
        if(this.ent_editor_wrapper) this.ent_editor_wrapper.remove();
        this.wrapper.remove();
    ...
    // mode - full|small|icon - full is default mode
    _defineActionButton: function(action, container, mode, style){        
        ...
        if(mode=='icon'){
                ...
                var icons = (action.icon)?{primary: action.icon}:null;
    ...
    //  -----------------------------------------------------
    //  send update request and close popup edit dialog
    //
                    ...
                    function(response){
                        if(response.status == top.HAPI4.ResponseStatus.OK){
                            ...
                            if(that.options.edit_mode=='popup'){
                                that.ent_editor_wrapper.dialog('close')
                            }
    ...
    _initEditForm_continue: function(recID){
        ...
        if(recID>0){
            if(this.options.edit_need_load_fullrecord){
                ...
                if(!this._entityIDfield) return alert('Developer! Define fieldname for ID!!!');
    ...
    _afterInitEditForm: function(){
        ...
        if(this.options.edit_mode=='inline'){
            ...
            this._defineActionButton({key:'save',label:'Save', title:'', icon:'ui-icon-check'},
                        this.ent_editor,'full',{margin:'0 40%'});
        }
        ...
        if(this.options.edit_mode=='popup'){ //show in popup
            ...
            var btn_array = [{text:top.HR('Save'),
                        click: function() { that._saveEditAndClose(); }},
                              {text:top.HR('Close'), 
                    click: function() { that.ent_editor_wrapper.dialog('close'); }}]; 
             
            this.ent_editor_wrapper.dialog({
                autoOpen: true,

...
$.widget( "heurist.buttonsMenu", {
    ...
    */
    ...
    _createMenuButton: function(top_level){
        ...
        let icon_right = top_level.attr('data-icon');
        if(!icon_right){
            icon_right = 'ui-icon-carat-d';
        }
        icon_right = (icon_right!='none')?`<span class="ui-icon-right ui-icon ${icon_right}"></span>`:'';

        let link = $(`<a ${menuID} href="#" style="padding:2px 22px 2px ${right_padding} !important;${linkCss}" title="${menuTitle}">${icon_left}<span>${menuLabel}</span>${icon_right}</a>`);
    ...
    menuActionHandler: function(event, ui) {
        ...
        if(ui?.item){
            ele = ui.item; // Item selected from a submenu
        }else{
            ele = $(event.target); // Click on a top-level button
            if(ele.is('span')){
                ele = ele.parent(); // If a span inside a button is clicked, get the parent anchor
            }
        }
    ...
    _initActionItem: function(idx, item){
        ...
        if(!action_hint){ //localized version not found

...
$.widget( "heurist.profile", {
    ...
    _create: function() {
            ...
            select: function( event, ui ) {
                ...
                if(action == "menu-user-logout"){
                    ...
                    if($.isFunction($('body').tag_manager)){ //already loaded
                        ...
                        $.getScript(top.HAPI4.basePathV4+'hclient/widgets/tag_manager.js', function(){ showManageTags(); } );
                    }
                }else if(action == "menu-user-faceted"){
                    if($.isFunction($('body').search_faceted_wiz)){ //already loaded
                        showSearchFacetedWizard();
                    }else{
                        $.getScript(top.HAPI4.basePathV4+'hclient/widgets/ssearch/search_faceted_wiz.js', function(){ showSearchFacetedWizard(); } );
                    }
                }else if(action == "menu-user-svs"){
                    if($.isFunction($('body').svs_manager)){ //already loaded
                        showManageSavedSearches();
                    }else{
                        $.getScript(top.HAPI4.basePathV4+'hclient/widgets/svs_manager.js', function(){ showManageSavedSearches(); } );
                    }
                }else if(action == "menu-user-files"){
                    if($.isFunction($('body').file_manager)){ //already loaded
                        showManageFiles();
                    }else{
                        $.getScript(top.HAPI4.basePathV4+'hclient/widgets/profile/file_manager.js', function(){ showManageFiles(); } );
                    }
                }
        ...
        top.HEURIST4.ui.createRectypeSelect(this.select_rectype.get(0), null, false);
        ...
        this._on( this.btn_record, {
            click: function() {
                ...
                return false;
                //window.open(top.HAPI4.basePathV4 + "hclient/framecontent/recordEdit.php?db="+top.HAPI4.database, "_blank");
            }
        });
        this._on( this.select_rectype, {
            click: function(event) {
                ...
                /* open in new window
                window.open(top.HAPI4.basePathV4 + "hclient/framecontent/recordEdit.php?db="+top.HAPI4.database+"&rt="+recordtype, "_blank");
                */
            }
        });
    ...
    */
    ...
    login: function(){
        ...
        if(  this.login_dialog.is(':empty') )
        {
            ...
            //load login dialogue
            $dlg.load(top.HAPI4.basePathV4 + "hclient/widgets/profile/profile_login.html?t="+(new Date().getTime()), function(){
                    ...
                    if(isreset){
                        ...
                        if(top.HEURIST4.msg.checkLength( rusername, "username", message, 1, 0 ))
                        {
                            top.HAPI4.SystemMgr.reset_password({username: rusername.val()}, function(response){
                                if(response.status == top.HAPI4.ResponseStatus.OK){
                                    $dlg.dialog( "close" );
                                    top.HEURIST4.msg.showMsgDlg(top.HR('Your password has been reset. You should receive an email shortly with your new password'), null, "Password reset");
                                }else{
                                    top.HEURIST4.msg.showMsgErr(response);
                                }
                            });
    ...
    editProfile: function()
    {
        if($.isFunction($('body').profile_edit)){
            ...
            var that = this;
            $.getScript(top.HAPI4.basePathV4+'hclient/widgets/profile_edit.js', function() {
                if($.isFunction($('body').profile_edit)){
                    that.editProfile();
                }else{
    ...
    */
    editPreferences: function()
    {
        ...
        $dlg.load(top.HAPI4.basePathV4+"hclient/widgets/profile_preferences.html", function(){

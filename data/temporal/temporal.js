 ...
 *	for the type.
 *
 *	@param	str		a string which is checked for validity
...
TDate.parse = function () {
		...
		if (t && t[0] && t[0] !== "T") { // possible word for month
			var word = temp.match(/^\s*(?:\d+\s)*\s*([^\.\-\d:\s\+\/]{3,20}\.?)\s*(?:\d+\s)*/);
			var valid = false;
			if (word) {
				word = word[1];
				var m = word.match(/jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec\.?/i);
				if (m) {
					temp = temp.replace( word, m[0].toLowerCase());
				} else {
...
var TDuration = function (strDuration) {
	...
	function _parseStr (str) {
		...
		if (str.replace(/[PYMD]T/,"T").match(/((\D\D)|([^PYMDTHS0-9]))/g)) { // catch any double non digital characters or any non ISO Dur standard characters

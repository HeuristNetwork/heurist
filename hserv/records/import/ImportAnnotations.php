...
class ImportAnnotations{
    ...
    private function validateImportModeForManifest($json): bool
    {
        ...
        // Overlay output preserves source Canvas identifiers and emits a Heurist
        // IIIF Presentation v3 overlay Manifest.  Presentation API v2 Manifests
        // are therefore imported only in full-management mode.
    ...
    private function extractAnnotationsV2($manifest, $baseUrl){
        ...
        foreach((array)@$manifest['sequences'] as $seq){
            foreach((array)@$seq['canvases'] as $canvas){
                ...
                // Standard IIIF Presentation v2 annotation lists.
                foreach((array)@$canvas['otherContent'] as $list){
                    $out = array_merge($out, $this->annotationsFromPageOrRef($list, $canvasId, $baseUrl));
                }

                // Compatibility: some legacy / generator-produced v2 Manifests
                // incorrectly place v3-style AnnotationPage objects in
                // canvas.annotations.  Import them as Web Annotations rather than
                // silently ignoring them.  IDs in this hybrid format are often
                // reused on every Canvas, so make them Canvas-scoped before save.
                foreach((array)@$canvas['annotations'] as $page){
                    $out = array_merge($out, $this->annotationsFromPageOrRef($page, $canvasId, $baseUrl, true));
                }
            }
        }
        return $out;
    ...
    private function annotationsFromPageOrRef($page, $canvasId, $baseUrl, bool $makeCanvasScopedIds=false){
        if(is_array($page) && (@$page['items'] || @$page['resources'])){
            if(@$page['type']=='AnnotationPage'){
                return $this->annotationsFromPage($page, $canvasId, $baseUrl, $makeCanvasScopedIds);
            }
        ...
        if(@$loaded['type']=='AnnotationPage'){
            return $this->annotationsFromPage($loaded, $canvasId, $url, $makeCanvasScopedIds);
        }
    ...
    private function annotationsFromPage($page, $canvasId, $sourcePageUrl, bool $makeCanvasScopedIds=false){
        $out = array();
        foreach((array)@$page['items'] as $idx=>$anno){
            ...
            $annotationId = $this->getJsonId($anno);

            if($makeCanvasScopedIds && $ctxCanvas){
                $localId = $annotationId ?: ('item-'.($idx+1));
                $annotationId = rtrim((string)$ctxCanvas, '#')
                    .'#annotation='.rawurlencode((string)$localId);

                // Store the normalised import identity in the JSON as well, so
                // DT_ORIGINAL_IIIF_ID, DT_ANNOTATION_INFO and re-import matching
                // all use the same stable Canvas-scoped identifier.

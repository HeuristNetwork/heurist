...
class RecordModify {
    ...
    /**
     * Retrieves default values for record creation, including Record Type, Owner Groups, 
     * Non-Owner Visibility, and Visibility Groups.
     *
     * This method attempts to get the default values for a new record based on:
     * - User preferences
     * - System settings
     * - Passed record data (if available)
     *
     * The method prioritizes user preferences but falls back to system defaults if preferences
     * are not set. If specific values are present in the provided record, they will override
     * defaults.
     *
     * @param array|null $record - (Optional) An associative array containing the record's data, 
     *                             where keys can be prefixed with 'rec_' or not.
     *                             Expected keys: 'RecTypeID', 'NonOwnerVisibility', 
     *                             'NonOwnerVisibilityGroups', 'OwnerUGrpID'.
     *
     * @return array - Returns an array with the following structure:
     *                 - 'rectype' => int, the Record Type ID
     *                 - 'owner_grps' => array, list of owner group IDs
     *                 - 'access' => string, non-owner visibility (e.g., 'viewable', 'restricted')
     *                 - 'access_grps' => string|null, groups allowed non-owner access (if applicable)
     */
    private function getDefaultsForNewRecord($record=null){
        ...
        // Get user preferences (array: record type, default owner, view mode, groups)
        $userDefaults = $this->getUserDefaultsForNewRecord();

        // Handle record-specific overrides if the record is provided
        if ($record !== null) {
            $record = $this->normalizeRecordArray($record);
            
            $rectype = isset($record['RecTypeID']) ? ConceptCode::getRecTypeLocalID($record['RecTypeID']) : null;
            
            $ownerid = record['OwnerUGrpID'] ?? -1;
            if ($ownerid == 'current_user') {
                $ownerid = $system->get_user_id();
            }
            $ownerid = prepareIds($ownerid, true);
            
            $access = $record['NonOwnerVisibility'] ?? null;
            $access_grps = $record['NonOwnerVisibilityGroups'] ?? null;
        }        

        // Apply defaults if no values are set
        $rectype = intval($rectype) ?: $userDefaults['rectype'];
        $owner_grps = $this->resolveOwnerGroups($ownerid, $userDefaults['owner_grps']);
        $access = $this->resolveAccess($access, $userDefaults['access']);
        
        if ($access != 'viewable') {
            $access_grps = null;
        }else{
            $access_grps = $access_grps ?? $userDefaults['access_grps'];        
        }

        return ['rectype' => $rectype, 'owner_grps' => $owner_grps, 'access' => $access, 'access_grps' => $access_grps];
    }        

    
    /**
     * Retrieves user default preferences for record creation.
     */
    private function getUserDefaultsForNewRecord() {
        $addRecDefaults = $this->system->user_GetPreference('record-add-defaults');
        ...
        $ownerGroup = $addRecDefaults[1] ?? null;
        if (!isEmptyStr($ownerGroup)) {
            $ownerGroup = explode(',', $ownerGroup);
        } elseif (is_numeric($ownerGroup)) {
            $ownerGroup = [intval($ownerGroup)];
        }else{
            $ownerGroup = null;
        }

        return [
            'rectype' => isPositiveInt($addRecDefaults[0] ?? null) ? intval($addRecDefaults[0]) : null,
            'owner_grps' => $ownerGroup,
            'access' => $addRecDefaults[2] ?? null,
            'access_grps' => $addRecDefaults[4] ?? null,
        ];
    } 
    
    /**
    * Resolves the Owner Group IDs, prioritizing record values over defaults.
    */
    private function resolveOwnerGroups($ownerid, $defaultOwnerGroups) {
        
        if ($ownerid == -1 || count($ownerid) == 0) {
            $ownerid = $defaultOwnerGroups ?: $this->system->get_system('sys_NewRecOwnerGrpID');
        }
        ...
        return is_array($ownerid) ? $ownerid : [$ownerid];
    }

    /**
    * Resolves the access level (non-owner visibility), falling back to system or user defaults.
    */
    private function resolveAccess($access, $defaultAccess) {
        if ($access === null) {
            $access = $defaultAccess ?: $this->system->get_system('sys_NewRecAccess');
        }
    ...
    /**
     * Normalizes the record array by stripping the 'rec_' prefix from keys.
     */
    private function normalizeRecordArray($record) {
...
function recordAdd($system, $record, $return_id_only=false){
    ...
    $def_params = $this->getDefaultsForNewRecord($record);
...
/**
* get all incremeneted value for given record type   (used in importAction)
*
* @param mixed $system
* @param mixed $params
*/
...
$dtl_Value = preg_replace('#<([A-Z][A-Z0-9]*)(\s*)(?:(?:(?:(?!'.$allowed2.$regex.$allowed
                ...
                case "date":
                    ...
                    if(!$isValid ){
                        ...
                        $dtl_Value = Temporal::getValueForRecDetails( $dtl_Value, true );
...
function recordDuplicate($system, $id){
    ...
    $def_params = $this->getDefaultsForNewRecord();
    ...
    if (!(isPositiveInt($new_owner) || intval($new_owner)==0)){   //current user is not member of current group

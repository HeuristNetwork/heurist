...
class FairScore {
    ...
    public static function computeForDatabase($mysqli, $database_name_full, $short_name, $filestore_root){
        ...
        $row = mysql__select_row_assoc($mysqli,
            "SELECT sys_dbRegisteredID, sys_dbName, sys_dbOwner, sys_dbRights, sys_dbDescription ".
        ...
        // DOI stored in /settings/DOIs.json per Ian Johnson's design (avoids schema changes and
        // is more flexible for storing multiple identifiers in future)
        $dois_file = rtrim($filestore_root, '/') . '/' . $short_name . '/settings/DOIs.json';
        $has_doi = false;
        if(file_exists($dois_file) && is_readable($dois_file)){
            $dois = json_decode(file_get_contents($dois_file), true);
            // accept any of the common keys a DOI might be stored under
            $has_doi = !empty($dois['database_doi']) || !empty($dois['doi']) || !empty($dois['database']);
        }

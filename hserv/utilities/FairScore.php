...
class FairScore
{
    ...
    public const SCORE_FILENAME = 'FAIRscore.txt';
    ...
    public static function computeForDatabase($mysqli, $database_name_full, $short_name, $filestore_root)
    {
        ...
        // ---- Accessible ---------------------------------------------------------------------------
        $total_records = intval(mysql__select_value(
            $mysqli,
            "SELECT count(*) FROM $db.Records WHERE rec_FlagTemporary=0",
        ));

        $public_records = intval(mysql__select_value(
            $mysqli,
        ...
        $rty_cms_home = mysql__select_value(
            $mysqli,
            "SELECT rty_ID FROM $db.defRecTypes WHERE rty_OriginatingDBID=99 AND rty_IDInOriginatingDB=51",
        );
        $rty_cms_page = mysql__select_value(
            $mysqli,
            "SELECT rty_ID FROM $db.defRecTypes WHERE rty_OriginatingDBID=99 AND rty_IDInOriginatingDB=52",
        );

        $has_website = isPositiveInt($rty_cms_home) && intval(mysql__select_value(
            $mysqli,
            "SELECT count(*) FROM $db.Records WHERE rec_RecTypeID=" . intval($rty_cms_home),
        )) > 0;

        $page_count = isPositiveInt($rty_cms_page) ? intval(mysql__select_value(
            $mysqli,
            "SELECT count(*) FROM $db.Records WHERE rec_RecTypeID=" . intval($rty_cms_page),
        )) : 0;

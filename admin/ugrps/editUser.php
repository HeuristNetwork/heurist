...
/*****DEBUG****///error_log("in editUser before verify");

$body->verify();
if (@$_REQUEST['_submit']) {
	$usr_id = intval(@$_REQUEST['Id']);

/*****DEBUG****///error_log("in editUser in Submit");
...
";
/*****DEBUG****///error_log("sending user confirmation mail: " . $email . ", " . $row['ugr_FirstName'].' '.$row['ugr_LastName'].'['.$row['ugr_eMail'].']');

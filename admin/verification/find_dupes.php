...
if (@$_REQUEST['personmatch']){
    $personMatch = true;
    $res = mysql_query('select rec_id, rec_type, rec_title, rd_val from records left join rec_details on rd_rec_id=rec_id and rd_type=291 where rec_type = 55 and not rec_temporary order by rec_id desc');    //Given Name
    while ($row = mysql_fetch_assoc($res)) {
       $recsGivenNames[$row['rec_id']] = $row['rd_val'];
    }
    $res = mysql_query('select rec_id, rec_type, rec_title, rd_val from records left join rec_details on rd_rec_id=rec_id and rd_type=160 where rec_type = 55 and not rec_temporary order by rd_val asc');    //Family Name
...
foreach ($dupes as $rectype => $subarr) {
    foreach ($subarr as $index => $key) {
	    ...
	    print '<a target="_new" href="'.HEURIST_URL_BASE.'search/heurist-search.html?w=all&q=ids:' . join(',', array_keys($bibs[$key])) . '">search</a>&nbsp;&nbsp;&nbsp;&nbsp;';
	    ...
	    foreach ($bibs[$key] as $rec_id => $vals) {
		    ...
		    print '<li>'.($crosstype ? $vals['type'].'&nbsp;&nbsp;' : '').
		    		'<a target="_new" href="'.HEURIST_URL_BASE.'data/records/viewrec/view.php?saneopen=1&bib_id='.$rec_id.'">'.$rec_id.': '.htmlspecialchars($vals['val']).'</a>';
		    if ($row['rec_url'])

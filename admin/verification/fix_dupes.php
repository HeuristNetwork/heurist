...
//    foreach($records as $index) {
    foreach($records as $record) {
      //  $record = $records[$index];
        ...
        print '<span style="font-size: 120%;"><a target="edit" href="'.HEURIST_URL_BASE.'records/editrec/heurist-edit.html?bib_id='.$record['rec_id'].'">'.$record['rec_id'] . ' ' . '<b>'.$record['rec_title'].'</b></a> - <span style="background-color: #FFDDDD;">'. $rfts[$record['rec_type']].'</span></span>';
	    print '<table>';
	    ...
	    if ($record["refs"]) {
		    ...
		    foreach ($record["refs"] as $ref) {  //FIXME  check for reference to be a valid record else mark detail for delete and don't print
			    print '<a target="edit" href="'.HEURIST_URL_BASE.'records/editrec/heurist-edit.html?bib_id='.$ref.'">'.$i++.'</a> ';
		    }
		    print '</td></tr>';
	    }
    ...
    foreach($rec_keys as $index) {
        ...
        print '<td style="width: 500px;">';
        print '<div style="font-size: 120%;"><a target="edit" href="'.HEURIST_URL_BASE.'records/editrec/heurist-edit.html?bib_id='.$record['rec_id'].'">'.$record['rec_id'] . ' ' . '<b>'.$record['rec_title'].'</b></a> - <span style="background-color: #FFDDDD;">'. $rfts[$record['rec_type']].'</span></div>';
        print '<table>';
	...
	if (in_array($rd_type, array_keys($reference_bdts))) {
		if (is_array($rd_val)) {
			foreach ($rd_val as $val){
                ...
                $rv[] = '<a target="edit" href="'.HEURIST_URL_BASE.'records/editrec/heurist-edit.html?bib_id='.$val.'">'.$title['rec_title'].'</a>';
            }
			return $rv;
		}
		else {
            $title = mysql_fetch_assoc(mysql_query('select rec_title from records where rec_id ='.$rd_val));
			return '<a target="edit" href="'.HEURIST_URL_BASE.'records/editrec/heurist-edit.html?bib_id='.$rd_val.'">'.$title['rec_title'].'</a>';
		}
	}
	/*
	else if ($rd_type == 158) {

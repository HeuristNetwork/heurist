...
-- ** New triggers for vocab groups added, based on the above, by Ian 28/8/2020

DELIMITER $$

--              insert
    DROP TRIGGER IF EXISTS defVocabularyGroups_insert$$

    CREATE
    DEFINER=CURRENT_USER 
    TRIGGER `defVocabularyGroups_insert`
    AFTER INSERT ON `defVocabularyGroups`
    FOR EACH ROW
        update sysTableLastUpdated set tlu_DateStamp=now() where tlu_TableName="defVocabularyGroups"$$

DELIMITER ;
DELIMITER $$

--              update
    DROP TRIGGER IF EXISTS defVocabularyGroups_update$$

    CREATE
    DEFINER=CURRENT_USER 
    TRIGGER `defVocabularyGroups_update`
    AFTER UPDATE ON `defVocabularyGroups`
    FOR EACH ROW
        update sysTableLastUpdated set tlu_DateStamp=now() where tlu_TableName="defVocabularyGroups"$$
...
--              delete
    DROP TRIGGER IF EXISTS defVocabularyGroups_delete$$

    CREATE
    DEFINER=CURRENT_USER 
    TRIGGER `defVocabularyGroups_delete`
    AFTER DELETE ON `defVocabularyGroups`
    FOR EACH ROW
        update sysTableLastUpdated set tlu_DateStamp=now() where tlu_TableName="defVocabularyGroups"$$

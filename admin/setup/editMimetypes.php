<?php
    ...
    // Edit the mime types table for this database
    // Ian Johnson 11 Jan 2010
    // Copyright (C) 2010 University of Sydney, Digital Innovation Unit
    // Coded with NuSphere PHP-Ed
    
    // TO DO: should this table be in teh INSTALLATION rather than in each individual database?
    //        potential for inconsistency if not in the individual databases
    ...
    if (! is_admin()) {
        print "<html><body><p>You must be an adminstrator to edit the mime types table</p><p><a href='".HEURIST_URL_BASE."common/connect/login.php?logout=1&amp;instance=".HEURIST_INSTANCE."'>Log out</a></p></body></html>";
        return;
    }
...
define('MSG_UPDATED', "Record has been updated successfully.
    Click <a href=\"?mode=s&amp;start=%d\">here</a> to return to list.");       
...
$table = 'defFileExtToMimetype';
$scheme = '';
$fielddef = array(
    'f0' => array(FLD_ID => true, FLD_VISIBLE => true, FLD_DISPLAY => 'Extension (several may share same mime type)', FLD_DISPLAY_SZ => 5,
        ...
        FLD_DATABASE => 'fxm_Extension'),
    'f1' => array(FLD_ID => false, FLD_VISIBLE => true, FLD_DISPLAY => 'Mime type (general type/specific type)', FLD_DISPLAY_SZ => 50,
        FLD_INPUT => true, FLD_INPUT_TYPE => 'text',
    ...
    'f2' => array(FLD_ID => false, FLD_VISIBLE => true, FLD_DISPLAY => 'Open this file type in new window (1 = yes, 0 = no)', FLD_DISPLAY_SZ => 2,
        ...
        FLD_DATABASE => 'fxm_OpenNewWindow'),
    'f3' => array(FLD_ID => false, FLD_VISIBLE => true, FLD_DISPLAY => 'Filename of icon for files with this extension', FLD_DISPLAY_SZ => 31,
        FLD_INPUT => true, FLD_INPUT_TYPE => 'text',
        FLD_INPUT_SZ => 31, FLD_INPUT_MAXLEN => 31, FLD_INPUT_DFLT => '',
        FLD_INPUT_NOTEMPTY => false, FLD_INPUT_VALIDATION => '',
        FLD_DATABASE => 'fxm_IconFileName'),
    'f4' => array(FLD_ID => false, FLD_VISIBLE => true, FLD_DISPLAY => 'Descriptive name for this file type eg. "JPEG image"', FLD_DISPLAY_SZ => 31,
        FLD_INPUT => true, FLD_INPUT_TYPE => 'text',
        FLD_INPUT_SZ => 31, FLD_INPUT_MAXLEN => 31, FLD_INPUT_DFLT => '',
        FLD_INPUT_NOTEMPTY => false, FLD_INPUT_VALIDATION => '',
        FLD_DATABASE => 'fxm_FiletypeName'),
    'f5' => array(FLD_ID => false, FLD_VISIBLE => true, FLD_DISPLAY => 'Filename of generic image (when thumbnail cannot be generated)', FLD_DISPLAY_SZ => 64,
        FLD_INPUT => true, FLD_INPUT_TYPE => 'text',
        FLD_INPUT_SZ => 31, FLD_INPUT_MAXLEN => 64, FLD_INPUT_DFLT => '',
        FLD_INPUT_NOTEMPTY => False, FLD_INPUT_VALIDATION => '',
        FLD_DATABASE => 'fxm_ImagePlaceholder')
);
...
<tr>
    ...
    <!-- TO DO: Needs a Cancel button which closes window -->
</tr>
</table>

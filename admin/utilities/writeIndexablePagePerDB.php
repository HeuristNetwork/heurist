...
*/
    ...
    $page_content_id = mysql__select_value($mysqli, 'SELECT dty_ID FROM defDetailTypes WHERE dty_OriginatingDBID = 2 AND dty_IDInOriginatingDB = 4');
    ...
    if($cms_home_id !== null && $page_content_id !== null){
 ...
 // OLD VERSION, onluy looks at home page
 //               $checkQuery = <<<QUERY
 //               SELECT dtl_ID
 //               FROM recDetails
 //               WHERE dtl_RecID = {$rec_ID} AND dtl_DetailTypeID = {$page_content_id} AND dtl_Value NOT LIKE '%/////This is default content generated by the Heurist CMS function%'
 //               QUERY;
//
//                $hasDefaultContent = mysql__select_value($mysqli, $checkQuery);
//                if($hasDefaultContent === null){
//                    continue;
//                }

// NEW VERSION
                // Exclude websites containing two or more directly-linked pages
                // with default/unconfigured CMS content.
                $checkQuery = <<<QUERY
                SELECT COUNT(DISTINCT content.dtl_RecID)
                FROM recDetails content
                WHERE content.dtl_DetailTypeID = {$page_content_id}
                  AND (
                       content.dtl_Value LIKE '%This is default content generated by the Heurist CMS function%'
                    OR content.dtl_Value LIKE '%This is an unconfigured page%'
                    OR content.dtl_Value LIKE '%Use this page to describe%'
                    OR content.dtl_Value LIKE '%Use this page for a description%'
                  )
                  AND (
                       content.dtl_RecID = {$rec_ID}
                       OR content.dtl_RecID IN (
                           SELECT ptr.dtl_Value
                           FROM recDetails ptr
                           WHERE ptr.dtl_RecID = {$rec_ID}
                             AND ptr.dtl_Value REGEXP '^[0-9]+$'
                       )
                  )
                QUERY;

                $defaultPageCount = intval(mysql__select_value($mysqli, $checkQuery));

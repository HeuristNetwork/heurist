...
set_time_limit(0); // No time limit for execution to avoud timeout
...
*  unchanged for more than a year with more than 200 records
*/
    ...
    /*  ??? WHY HAVE THESE TESTS BEEN TAKEN OUT ???
    ...
    */
    ...
    $interval = date_diff($datetime1, $datetime2); // inactivity time
    $diff = $interval->format('%y')*12 + $interval->format('%m');

    // Check inactivity time against number of records to decide if ot should be purged
...
/*  
* Dump and bz2 any import tables that are:
*     more than 2 months old
*     older than 1 month if more than 10 tables
*     reduce to 20 most recent tables if more than 20 left
...
*/
        ...
        if(ALLOW_ARCHIVE_SYSARCHIVE){ //allow archive sysArchive

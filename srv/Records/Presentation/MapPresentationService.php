...
use Heurist\Runtime\ConceptCode;
...
class MapPresentationService
{
    ...
    private PresentationRecordRepository $documents;
    private PresentationRecordRepository $layers;
    private PresentationRecordRepository $terms;
    private ConceptCode $conceptCodes;

    /**
     * Initialise the map presentation service.
     *
     * @param PresentationRecordRepository $records Direct read-only record access.
     * @param RuntimeContext $runtime Current database and URL context.
     * @param ConceptCode $conceptCodes Stable term-code lookup.
     */
    ...
    {
        $record = $this->documents->getPublicRecord($recordId, 'RT_MAP_DOCUMENT');
        ...
        foreach($layerIds as $index=>$layerId){
            $layer = $this->layers->getPublicRecord($layerId, 'RT_MAP_LAYER');
            if(!$layer){ continue; }
            $layers[] = array(
                'id' => $layerId,
                'recordId' => $layerId,
    ...
    {
        $layer = $this->layers->getPublicRecord($recordId, 'RT_MAP_LAYER');
    ...
    private function buildSource(array $record): array
    {
        ...
        if($type=='remote-geojson'){
            ...
            $source['url'] = $this->runtime->baseUrl.'api/'
                .rawurlencode($this->runtime->databaseName).'/map/data/'
                .intval($record['rec_ID']); //.'?format=geojson'; geojson is default output
        }elseif($type=='tile'){
            
            $fileId = $this->layers->value($record, 'DT_SERVICE_URL');
            $fileinfo = $this->layers->getUploadedFile($fileId);
            if(!empty($fileinfo)){
                ...
                if(!(strpos($url,'{q}')>0 || strpos($url,'{z}/{x}/{y}')>0)){
                    ...
                    if($mimeType>0){
                        $mimeType = $this->conceptCodes->getTermConceptId($mimeType);
            ...
            //TMS naming schema
            $tmsSchema = $this->conceptCodes->getTermLocalId('2-548');
            ...
            $fileinfo = $this->layers->getUploadedFile($file);
            if(!empty($fileinfo)){
                $url = @$fileinfo['ulf_ExternalFileReference'];    
                if(!$url){
                    $fileIdObfuscated = @$fileinfo['ulf_ObfuscatedFileID'];
                    $url = $this->runtime->baseUrl.'?fullres=1&db='.
                        rawurlencode($this->runtime->databaseName).'&file='.$fileIdObfuscated;
                }
            }
            $source['bounds'] = $this->normaliseGeo($this->layers->value($record, 'DT_GEO_OBJECT'));
            $source['url'] = $url;

<?php
/**
* Temporal.php - Temporal conversion utilities for the srv layer
* 
...
class Temporal {
    ...
    private const REGEX_YEARONLY = '/^-?\d+$/';
    ...
    private static function _parseTemporal( $value, $is_for_search=false ){
        ...
        if(is_array($value) && (@$value['timestamp'] || @$value['start'])){
            ...
            if($timespan==null && !preg_match(self::REGEX_YEARONLY, $value) ){
                ...
                if($timespan){
                    if(is_double($timespan)){ //200.15
                        ...
                        $timespan = null;
                    }elseif(is_array($timespan) && !empty($timespan) && is_numeric(@$timespan[0])){
                        if(count($timespan)==1){
    ...
    private static function _getInterval($timestamp, $deviation, $direction=0){
        ...
        $is_year_only = ($deviation==null || preg_match('/^P\d+Y$/',$deviation)) && preg_match(self::REGEX_YEARONLY,$timestamp);
        ...
        if(is_array($date)){
            if(@$date['year']!=null){
                ...
                if(($date['month'] ?? 0)>0){
                    ...
                    if(($date['day'] ?? 0)>0 && !empty($date['has_days'])){
    ...
    private static function _datePrepare($value, $month_day_order=2){
        ...
        if(!preg_match(self::REGEX_YEARONLY, $value) && $value[0] == '-'){ //this is BCE with month and day
        ...
        if( preg_match(self::REGEX_YEARONLY, $value) ){ //this is YEAR - only digits with possible minus and spaces for milles
    ...
    public function toReadableExt($separator, $is_compact=false, $out_calendar=null){
        ...
        if($this->tDate){
            ...
            if($is_compact){
                ...
                $supinfo = array();
                if(!empty($res['Determination'])) {$supinfo[] = $res['Determination'];}
                if(!empty($date['calibrated'])) {$supinfo[] = 'Calibarated';}

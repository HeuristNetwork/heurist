...
<?php
	function delete_bib($rec_id) {
		...
		$res = mysql_query('select pers_id from personals where pers_rec_id = ' . $rec_id);
		while ($row = mysql_fetch_assoc($res))
			mysql_query('delete from keyword_links where kwl_pers_id = ' . $row['pers_id']);
		mysql_query('delete from personals where pers_rec_id = ' . $rec_id);
	...
	if (@$_REQUEST['delete'] == 1) {
		...
		foreach ($_REQUEST['bib'] as $rec_id) {
			...
			$res = mysql_query('select '.USERS_USERNAME_FIELD.' from records left join personals on pers_rec_id=rec_id left join '.USERS_DATABASE.'.'.USERS_TABLE.' on '.USERS_ID_FIELD.'=pers_usr_id where rec_id = ' . $rec_id);
...
<?php
	...
	foreach ($bib_ids as $rec_id) {
		...
		print ' ' . $rec_id . '<a target=_new href="'.HEURIST_SITE_PATH.'data/records/editrec/heurist-edit.html?bib_id='.$rec_id.'"><img src='.HEURIST_SITE_PATH.'common/images/external_link_16x16.gif></a>' ."\n";
		print ' ' . $rec_title ."</p>\n";
		...
		if ($refs) {
			...
			while ($row = mysql_fetch_assoc($refs_res)) {
				print '  <a target=_new href="'.HEURIST_SITE_PATH.'data/records/editrec/heurist-edit.html?bib_id='.$row['rd_rec_id'].'">'.$row['rd_rec_id'].'</a>'."\n";
			}
			print "\n </p>\n";
		}

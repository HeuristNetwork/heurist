...
function add_keywords_by_id() {
	...
	if ($_REQUEST["kwd_ids"]) {
		...
		mysql_query('insert ignore into keyword_links (kwl_rec_id, kwl_pers_id, kwl_kwd_id) '
				  . 'select rec_id, bkm_ID, kwd_id from keywords, '.USERS_DATABASE.'.UserGroups, records left join usrBookmarks on rec_id=pers_rec_id and bkm_UGrpID= '.get_user_id()
...
function remove_keywords_by_id() {
	...
	if ($_REQUEST["kwd_ids"]) {
		...
		mysql_query('delete keyword_links from usrBookmarks'
		         ...
		         . ' where pers_rec_id in (' . join(',', $bib_ids) . ') and bkm_UGrpID = ' . get_user_id()
		         . ' and kwd_id in (' . join(',', $keywords) . ')');
		$keyword_count += mysql_affected_rows();
...
function bookmark_references() {
	...
	$new_bib_ids = mysql__select_array('records left join usrBookmarks on pers_rec_id=rec_id and bkm_UGrpID='.get_user_id(),
	                                   'rec_id', 'bkm_ID is null and rec_id in (' . join(',', $bib_ids) . ')');
	$existing_bkmk_ids = mysql__select_array('records left join usrBookmarks on pers_rec_id=rec_id and bkm_UGrpID='.get_user_id(),
	                                   'concat(bkm_ID,":true")', 'bkm_ID is not null and rec_id in (' . join(',', $bib_ids) . ')');

	if ($new_bib_ids) {
		mysql_query('insert into usrBookmarks
		                  (bkm_UGrpID, bkm_Added, bkm_Modified, pers_rec_id)
		                  select ' . get_user_id() . ', now(), now(), rec_id
		                    from records where rec_id in (' . join(',', $new_bib_ids) . ')');
		$inserted_count = mysql_affected_rows();
		$bkmk_ids = mysql__select_array('usrBookmarks', 'concat(bkm_ID,":true")', 'pers_rec_id in ('.join(',',$new_bib_ids).') and bkm_UGrpID = ' . get_user_id());
	} else {
		$inserted_count = -1;
	}
...
function bookmark_and_tag_bibids ( $phpReturn ) {
	...
	$bkmk_ids = mysql__select_array('usrBookmarks', 'bkm_ID', 'pers_rec_id in ('.join(',',$bib_ids).') and bkm_UGrpID = ' . get_user_id());

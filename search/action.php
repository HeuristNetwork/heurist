...
function add_keywords() {
	...
	if (trim($_REQUEST["keywordstring"])) {
		...
		mysql_query('insert ignore into keyword_links (kwl_pers_id, kwl_rec_id, kwl_kwd_id) '
				  . 'select bkm_ID, bkm_recID, kwd_id from usrBookmarks, keywords '
...
function add_keywords_by_id() {
	...
	if ($_REQUEST["kwd_ids"]) {
		...
		mysql_query('insert ignore into keyword_links (kwl_rec_id, kwl_pers_id, kwl_kwd_id) '
				  . 'select rec_id, bkm_ID, kwd_id from keywords, '.USERS_DATABASE.'.UserGroups, records left join usrBookmarks on rec_id=bkm_recID and bkm_UGrpID= '.get_user_id()

...
function Filter(paramArray) {
    if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.entry("Filter"); }
    ...
    if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.exit("Filter"); }
        return that;
}
rBrowser.workflowIndex = 0;
function calcZOrder (marker,b) {
    if (app.displayManager.getFocus().getTitle() == marker.getTitle()) return 20;
...
function RecordMarker(record,manager) {
    ...
    if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.entry("RecordMarker",record.getID()); }
     ...
     /**********private***************/
        ...
        function _createMarker(baseIcon, loc){
                ...
                if (orgType  &&  orgType[0]) {
                    ...
                    recIcon.image = "http://heuristscholar.org/relmap/img/reftype/png/" + _hRecord.getRecordType().getID() + ".png";
                }
     ...
     if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.entry("marker setDisplayLocation",("location:" + location)); }
                                    if (location){
                                            var loc = _gMarker.getLatLng();
                                            if (loc.lat() == location.lat() && loc.lng() == location.lng()){
                                                 if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.exit("marker setDisplayLocation",("location:" + location)); }
                                                return;   //same location we are done
                                            }
                                     }else{
     ...
     if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.exit("marker setDisplayLocation",("location:" + location)); }
      ...
      if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.entry("marker onClickHandler",("ID : " + this.getID())); }
     ...
     if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.exit("marker onClickHandler",("ID : " + this.getID())); }
                },
            isAdded: function() { return _added;},
            hide: function(){
                                if (_gMarker != null && _added){
                                    // this marker got a message to hide, let the cluster manager know so it can adjust
...
//                                _sendHide();
            ...
            show: function(){
     if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.entry("marker show"); }
                                if (_gMarker == null){
                                    if (! _createMarker()){
                                    if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.exit("marker show"); }
                                        return;
                                    }
                                }
                                if (_filtered) {
                                ...
                                _gMarker.show();
                                // this marker got a message to show, let the cluster manager know so it can adjust
                                if (that.clusterGroupMarker) that.clusterGroupMarker.markerVisibilityUpdate(that, true);
                                // notify the relation markers
                                _sendShow();
     if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.exit("marker show"); }
            },
            isHidden: function() {
    ...
    if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.exit("RecordMarker",record.getID()); }
        return that;
}
...
function RelationMarker(record, manager) {
    ...
    if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.entry("RelationMarker", record.getID()); }
     ...
     if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.entry("relation marker _getVertices"); }
            ...
            vertices.push(new GLatLng(parseFloat(dLoc.lat()),parseFloat(dLoc.lng())));
     if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.exit("relation marker _getVertices"); }
            return vertices;
        }
...
/*                        len = newVertices.length,
  ...
  */
       ...
       if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.entry("relation marker show", sendingMarker.getID()); }
                              if (_gPolyline == null)
                                    if (! _createLine()){
                                        if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.exit("relation marker show", sendingMarker.getID()); }
                                        return;
                                    }
                                if (_filtered){
                                    if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.exit("relation marker show", sendingMarker.getID()); }
    ...
    if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.exit("RelationMarker",record.getID()); }
        return that;
}
...
function AffinityGroupMarker(affinityMarker,manager) {
    ...
    if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.entry("AffinityGroupMarker",affinityMarker.getID()); }
     ...
     if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.entry("AffinityGroup.onAffinityMarkerMove",_affinityMarker.getID()); }
                if (_expanded) {
                    this.recalcLayout();
                }
                else {
     ...
     if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.exit("AffinityGroup.onAffinityMarkerMove",_affinityMarker.getID()); }
            },

            onClickHandler : function() {
     if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.entry("AffinityGroup.onClickHandler",_affinityMarker.getID()); }
                if (! _expanded) {
                    this.expand();
                }
                else {
     ...
     if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.exit("AffinityGroup.onClickHandler",_affinityMarker.getID()); }
            },
    ...
    if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.exit("AffinityGroupMarker",affinityMarker.getID()); }
        return that;
}
...
function ClusterMarker(loc,index,manager) {
    if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.entry("ClusterMarker",index); }
    ...
    var that = {
     ...
     if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.entry("Cluster.onClickHandler",index); }
           if (! _expanded) {
                this.expand();
            }
            else {
    ...
    if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.exit("Cluster.onClickHandler",index); }
        },
    ...
    if (typeof(JSTraceProfiler) != "undefined") { JSTraceProfiler.exit("ClusterMarker",index); }

...
//	if (defined('use_alt_db')  &&  $database == 'heuristdb') $database = 'heuristdb_alt';
	...
	mysql_query_('set character set "utf8"');
	mysql_query_('set names "utf8"');
...
//	if (defined('use_alt_db')  &&  $database == 'heuristdb') $database = 'heuristdb_alt';
	...
	if (function_exists('get_user_id'))
		mysql_query_('set @logged_in_user_id = ' . get_user_id());
...
if (defined("T1000_DEBUG")) error_log($stmt);
	...
	return mysql_query_($stmt);	// Maybe something bad, maybe something good!
}
...
function mysql__update($table, $condition, $pairs_assoc) {
	...
	return mysql_query_($stmt);	// Up to the user to check for error.
}
...
function mysql__select_array($table, $column, $condition) {
	...
	$res = mysql_query_("SELECT $column FROM $table WHERE $condition");
	if (! $res) return NULL;
...
function mysql__select_assoc($table, $key_column, $val_column, $condition) {
	...
	$res = mysql_query_("SELECT $key_column, $val_column FROM $table WHERE $condition");
	if (! $res) return NULL;

...
if (!is_logged_in()) {
	if (! (@$_REQUEST['bkmrk_bkmk_url'] or @$_REQUEST['bkmrk_bkmk_title'] or @$_REQUEST['bkmrk_bkmk_description']))
		...
		header('Location: ' . HEURIST_BASE_URL_V3 . 'common/connect/login.php?db='.HEURIST_DBNAME.'&bkmrk_bkmk_title='.urlencode($_REQUEST['bkmrk_bkmk_title']).'&bkmrk_bkmk_url='.urlencode($_REQUEST['bkmrk_bkmk_url']).'&bkmrk_bkmk_description='.urlencode($_REQUEST['bkmrk_bkmk_description']));
	return;
}
$usrID = get_user_id();
...
if (@$_REQUEST['bkmrk_bkmk_url']) {
	...
	if (mysql_num_rows($res) > 0) {
		...
		$bkm_ID = $bkmk['bkm_ID'];
        $url = HEURIST_BASE_URL_V3 . 'records/edit/editRecord.html?db='.HEURIST_DBNAME.'&bkmk_id='.$bkm_ID.'&fromadd=exists' . $outdate;
		header('Location: ' . $url);
		return;
	}
...
//if no record found check for similar url's
	if (! @$rec_id  &&  ! @$force_new) {
		if (exist_similar($url)) {
			...
			header('Location: ' . HEURIST_BASE_URL_V3 . 'records/add/disambiguateRecordURLs.php'
								. '?db='.HEURIST_DBNAME
...
//if no similar url's and recID was -1 then force a new record of rec_rectype supplied
	...
	if (!isset($rec_id)  ||  (@$force_new && $force_new)) {
		...
		if (! $rt) {
			...
			header('Location: ' . HEURIST_BASE_URL_V3 . 'records/add/addRecord.php'
								. '?db='.HEURIST_DBNAME
...
if ($rec_id  &&  ! @$_REQUEST['force_new']) {
	...
	if ($bkmk  &&  $bkmk['bkm_ID']) {
        ...
        $url = HEURIST_BASE_URL_V3 . 'records/edit/editRecord.html?db='.HEURIST_DBNAME.'&bkmk_id='.$bkmk['bkm_ID'].'&fromadd=exists' . $outdate . "#personal";
		header('Location: ' . $url );
		return;
	}
}
...
if ($rec_id) {
	...
	if ($bkm_ID) {
		if ($isNewRecID) {
            $url = HEURIST_BASE_URL_V3 . 'records/edit/editRecord.html?db='.HEURIST_DBNAME.'&bkmk_id=' . $bkm_ID . '&fromadd=new_bib' . $outdate . $wg;
			header('Location: ' . $url);
		} else {
            $url = HEURIST_BASE_URL_V3 . 'records/edit/editRecord.html?db='.HEURIST_DBNAME.'&bkmk_id=' . $bkm_ID . '&fromadd=new_bkmk' . $outdate . $wg;
			header('Location: ' . $url);
		}
		return;
	}

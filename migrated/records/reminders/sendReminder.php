...
function sendReminderEmail($reminder, $USERS_DATABASE, $HOST, $BASE_URL) {
	...
	if ($BASE_URL === NULL) $BASE_URL = HEURIST_BASE_URL_V3;
	...
	$email_title = '"'.$bib['rec_Title'].'"';

	if (@$reminder['rem_ToUserID'] != @$reminder['rem_OwnerUGrpID'])
		$email_title .= ' from ' . $owner['ugr_FirstName'].' '.$owner['ugr_LastName'];
	...
	foreach($recipients as $recipient) {
        ...
        sendEmail($recipient['email'], $email_title, $email_text, $email_headers, true);
	}

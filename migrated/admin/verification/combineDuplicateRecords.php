                ...
                <table><tbody id="tb">
                        ...
                        if (! @$do_merge_details){  // display a page to user for selecting which record should be the master record
                            ...
                            foreach($records as $record) {
                                ...
                                print '<span style="font-size: 120%;"><a target="edit" href="'.HEURIST_BASE_URL_V3.'records/edit/editRecord.html?db='.HEURIST_DBNAME.'&recID='.$record['rec_ID'].'">'.$record['rec_ID'] . ' ' . '<b>'.$record['rec_Title'].'</b></a> - <span style="background-color: #EEE;">'. $rtyNameLookup[$record['rec_RecTypeID']].'</span></span>';
                                print '<table>';
                                ...
                                if (array_key_exists("refs",$record)) {
                                    ...
                                    foreach ($record["refs"] as $ref) {  //FIXME  check for reference to be a valid record else mark detail for delete and don't print
                                        print '<a target="edit" href="'.HEURIST_BASE_URL_V3.'records/edit/editRecord.html?db='.HEURIST_DBNAME.'&recID='.$ref.'">'.$i++.'</a> ';
                                    }
                                    print '</td></tr>';
                                }
                            ...
                            foreach($rec_keys as $index) {
                                ...
                                print '<td style="width: 500px;">';
                                print '<div style="font-size: 120%;"><a target="edit" href="'.HEURIST_BASE_URL_V3.'records/edit/editRecord.html?db='.HEURIST_DBNAME.'&recID='.$record['rec_ID'].'">'.$record['rec_ID'] . ' ' . '<b>'.$record['rec_Title'].'</b></a> - <span style="background-color: #EEE;">'. $rtyNameLookup[$record['rec_RecTypeID']].'</span></div>';
                                print '<table>';
    ...
    if (in_array($rd_type, array_keys($reference_bdts))) {
        if (is_array($rd_val)) {
            foreach ($rd_val as $val){
                ...
                $rv[] = '<a target="edit" href="'.HEURIST_BASE_URL_V3.'records/edit/editRecord.html?db='.HEURIST_DBNAME.'&recID='.$val.'">'.$title['rec_Title'].'</a>';
            }
            return $rv;
        }
        else {
            $title = mysql_fetch_assoc(mysql_query('select rec_Title from Records where rec_ID ='.$rd_val));
            return '<a target="edit" href="'.HEURIST_BASE_URL_V3.'records/edit/editRecord.html?db='.HEURIST_DBNAME.'&recID='.$rd_val.'">'.$title['rec_Title'].'</a>';
        }
    }
    /*
    else if ($rd_type == 158) {

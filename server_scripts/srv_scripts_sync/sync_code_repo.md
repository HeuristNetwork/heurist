...
```bash
cp /path/to/your/source/sync_code_repo.sh /srv/scripts/sync_code_repo.sh
```

Adjust `/path/to/your/source/` to the actual directory where the script is stored in your environment.
...
### 5.4 Script contents (reference)

For reference, the script should look like this:
...
Confirm the contents if needed with:

```bash
cat /srv/scripts/sync_code_repo.sh
```

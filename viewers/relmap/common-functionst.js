...
function drawInputForm(detailType, recType){
	...
	if (isRepeatable)
	tD2.innerHTML = "<img id= \"img"+detailId +"\" src=\""+path+"common/images/duplicate.gif\" border=0>";
...
function drawUrlField(record){
	...
	function drawDateField(id, detail){
	  ...
	  var html ="";
		if (!isEmpty(detail)){
		  for (i in detail){
			theDate = new Date(detail[i]);
			theDate = theDate.getFullYear()+"-"+(theDate.getMonth()+1)+"-"+(theDate.getDate());

            html += "<div class=\"inp-div\"><input type=text  name=\""+id+"\" id=\""+id+"\" value=\""+theDate+"\">&nbsp<img src=\""+path+"common/images/calendar.gif\" id=\"calendar"+id+"\" onclick=\"window.open('calendar.html?id="+id+"','mywin','width=300, height=150, resizable=no');\"></div>";
		  }
		}else{
		  html += "<div class=\"inp-div\"><input type=text  name=\""+id+"\" id=\""+id+"\" value=\"\">&nbsp<img src=\""+path+"common/images/calendar.gif\" id=\"calendar"+id+"\" onclick=\"window.open('calendar.html?id="+id+"','mywin','width=300, height=150, resizable=no');\"></div>";
		}
	  sel.innerHTML += html;
	}
...
function saveRecord(record) {
       ...
       var saver = new HSaver(
			function(r) {
			alert ("Record saved");
			if ((window.opener.location.pathname==("/dos/addrelationship.html"))|| (window.opener.location.pathname==("/dos/edit-annotation.html"))) {

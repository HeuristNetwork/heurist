...
if (! window.heurist_images) window.heurist_images = new Array(
	...
	<xsl:for-each select="hml/records/record[type/@id=74][detail[@id=289]='image/jpeg' or detail[@id=289]='image/png' or detail[@id=289]='image/gif' or detail[@id=289]='image/bmp']">
		[ '<xsl:value-of select="detail[@id=221]/file/url"/>', "<xsl:value-of select="detail[@id=160]"/>" ]<xsl:if test="position() != last()">,</xsl:if>
	</xsl:for-each>
);

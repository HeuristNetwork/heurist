...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template name="ref_loop">
		...
		<xsl:variable name="refs" select="//hml/records/record"/>
		...
		<xsl:if test="$c &lt; $limit  and  $i &lt;= count($refs)">
			...
			<xsl:variable name="y" select="substring(/hml/dateStamp, 1, 4)"/>
			<xsl:variable name="m" select="substring(/hml/dateStamp, 6, 2)"/>
			<xsl:variable name="d" select="substring(/hml/dateStamp, 9, 2)"/>
				...
				<xsl:when test="($refdate/year &gt; $y)
					...
					<xsl:apply-templates select="$ref/detail[@id=177]"/>
						<xsl:if test="$ref/detail[@id = 223]">
							...
							<img src="{$ref/detail[@id = 223]/file/thumbURL}" alt="" align="middle"/>
							</div>
						</xsl:if>
						<xsl:if test="$ref/detail[@id = 221]">
							<div style="text-align: center;">
							<a href="{$ref/detail[@id = 221]/file/url}" target="_blank" style="font-size: 80%;">download pdf</a>
						              </div>
						</xsl:if>
					<hr/>
					</td></tr>

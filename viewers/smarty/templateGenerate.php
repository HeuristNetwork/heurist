...
//error_log(">>>>>>>>NOTHING FOUND");
	...
	//it is required to obtain name of header (coomon) fields only
...
function getRecordTypeSectionForSmarty($recTypeId, $parentName, $ind){
	...
	global $rtStructs, $recursion_depth;
...
/*
 ...
 $dtValue - record type structure definition
			...
			0"rst_DisplayName"    !!!! for label
			1"rst_DisplayHelpText"    !!!! for tooltip
			2"rst_DisplayExtendedDescription"
			3"rst_DefaultValue"
			4"rst_RequirementType"   will be required for loop???
			5"rst_MaxValues"
			6"rst_MinValues"
			7"rst_DisplayWidth"
			...
			12"rst_PtrFilteredIDs"  			 -- reference to record type for dtType==resource
...
function getDetailSectionForSmarty($parentName, $dtKey, $dtValue, $ind){
	...
	global $dtStructs, $rtStructs, $first_record, $recursion_depth;
	...
	if($dtDef){
			...
			$dt_label 	= $dtValue[0];  //rst_DisplayName
			$dt_tooltip = $dtValue[1]; //help text
...
//error_log(">>>>>>>".$dtKey."=".$dtNames[$dtKey].">>>".$dtname." TYPE=".$detailType);
			...
			*/
			case 'resource': // link to another record type
				...
				$pointerRecTypeId = $dtValue[12]; //@TODO!!!!! it may be comma separated string

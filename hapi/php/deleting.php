...
function deleteRecord($id) {
	...
	$res = mysql_query("select pers_id from records left join personals on pers_rec_id=rec_id where rec_id = " . $id . " and pers_id is not null");
	...
	if (is_admin()  ||  $owner === get_user_id()) {
		if ($reference_count === 0  &&  $bkmk_count === 0) {
			...
			mysql_query('delete from keyword_links where kwl_rec_id = ' . $id);
			if (mysql_error()) jsonError("database error - " . mysql_error());

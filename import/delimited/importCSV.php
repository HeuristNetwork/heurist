...
if($fields){
    $k=0;
?>    
    <div>Please check the list of columns read and select ones that contain dates (to be interpreted as a <b>date</b> field)<br><br></div>
    <table><!-- <tr> or extended text strings (to be interpreted as a <b>memo</b> field) <td>Unspecified</td><td>Memo</td><td>Date</td><td>&nbsp;</td></tr>-->
<?php        
...
onchange="{document.getElementById('u_field_<?=$k?>').checked = !this.checked; if(this.checked) document.getElementById('m_field_<?=$k?>').checked = false;}"     
-->    
    <td align="center"><input type="checkbox" id="d_field_<?=$k?>" name="datefield[]" value="<?=$k?>"/></td>
    <td><label><?=$field?></td>        
    </tr>  
<?php        
...
function doSelectSession(){
    ...
    document.forms[0].submit();
        ...
        <div class="help">This function loads a comma-separated or tab-separated text file, such as those generated by many spreadsheets and databases, allowing matching of rows against existing data based on one or more columns eg. name(s), dates, identifiers. Unmatched rows can be added as new records. The process can be repeated on the file to extract multiple entities from different columns and replace them with record IDs which can be used in a subsequent insertion or update of records.
        <br/><br/>
        Data rows must occupy a single line of data terminated with a CRLF (Windows) or an LF (Unix/Mac).  Linefeeds within memo fields should be represented by CR only. Fields should be separated by tab or comma. Fields containing the field separator should be enclosed in quotes. Quotes may exist within unquoted fields, but within quoted fields they should be preceded by a backslash ( \" ).
        <br/><br/>
        Editors such as <a href="http://notepad-plus-plus.org/" target="_blank">Notepad++</a>  (free, Open Source, Windows) show tabs, CR and LF as symbols and can do global replacements on them. 
        <br/><br/>
        Please visit the page on <a href="http://heuristnetwork.org/10-tips-for-using-heurist/importing-data/importing-delimited-text-files        " target="_blank">Importing delimited text files</a> on the Heurist network site for tips on sucessful import.
        <br/><br/>
        </div>
        ...
        <form action="importCSV.php" method="post" enctype="multipart/form-data" name="upload_form">
                ...
                <tr><td></td><td></td></tr>
                <tr><td align="right"><label>Select existing session:</label></td><td>
                     <select name="import_id" id="import_id" onchange="doSelectSession()"><?=get_list_import_sessions()?></select>
                     <input type="button" value="Clear All Sessions" style="font-weight: bold;" 
                                    onclick="doClearSession('all')">
                </td></tr>
                <tr><td align="right">Field separator:</td><td><select name="csv_delimiter"><option value="," selected>comma</option><option value="tab">tab</option></select></td></tr>

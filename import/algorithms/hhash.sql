delimiter //
CREATE FUNCTION hhash(bibID int) RETURNS varchar(4095)
    READS SQL DATA
    DETERMINISTIC
begin
	declare reftype int;
...
-- then concatenate them with beginning (^) and end ($) markers
	select group_concat(DST.bib_hhash order by bdt_id, bdt_id, DST.bib_hhash separator '$^')
...
-- If the reftype is null, we use "N" (for "private notes") instead.
	return concat(ifnull(reftype,'N'), ':',
...
update biblio set bib_hhash = hhash(bib_id) where bib_reftype in (44,10);
...
update biblio set bib_hhash = hhash(bib_id) where bib_reftype in (50,96);

#! /bin/sh
if [ -z $1 ] 
   then echo "Please supply version eg. h3.x.x or h3_alpha, h3_beta, h3_latest (this MUST exist as tar.bz2 on Google Code or script will not download Heurist code)" 
   exit
   fi
...
# SHOULD CHECK EXISTENCE OF   https://heurist.googlecode.com/files/$1.tar.bz2
...
echo "You may see a sudo error here; do not panic"
...
# Remove heuristscholar.org homepage - you can add your own
sudo rm index.html
...
echo -e "\n"
echo "Please edit configIni.php to set your MySQL root user + password (clearly documented in file)"
echo "You may also wish to increase the maximum file upload size in php.ini (see isntallation instructions)"
echo -e "\n\n"

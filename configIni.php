...
// 6.3.8  23 sep 2022 - download spreadsheet of file information for current filtered set of files, big cleanup of edit record code resulting in huge speed improvement for compelx records with many repeated term values, auto identificadtion of field matches by name in CSV import, improve New (record addition) shortcut on main screen, creation of new records from unrecognised H-ID numbers in CSV import, revise layout of filter icons nextto filter field on main screen Explore, error codes returned and shown when cannot access remote server, option to delete the base field when a field is deleted if it is unused, option to carry renaming and help text of field through to base field when changed, cleaner format for Record view, more readable no-visibility icon, single click navigation in lsit of fields in strucutre modification mode with doubkle-click to edit, network visualisation options to not show links and to reorganise as columns without gravity and saving of layout of visualisation in this mode, improve formatting of contrrols on website editing, render recognisable URLs in file description as hyperlinks, handle ISO formats in date import, handle remote URLs in multimedia import, drag up/down icon on values rollover in edit to tell people they can be reordered, 
// 6.3.6  08 Aug 2022 - allow use of bootstrap in custom reports, method for defining alternative language texts in multilingual websites, allow use of unprojected and alternative projection map data, direct annotation references on map widget including pixel coordiantes on tilestacks, better handling of ISO dates, Portuguese translation of interface, create new fields from a CSV file, option to delete all data as well as the field from a record type, improved handling of dates historgram in facet search, integrity check across all databases on server, 
// 6.3.5  13 July 2022 - script improvements, backup as a standalone and parameterisation, HeuristRef.net as reference server for Heuriverse, Add initial filters to map and custom report widgets, improve map zoom limit handling, improvements to IIIF partial image handling, zoom timing on Storymaps, calcualted fields using custom report to generate data, verification of remote fiel references at time of cration, auto purging of sysArchive table to zip file if over 50K records, databases_not_to_purge.txt file to protect reference databases from auto deletion when never updated, auto deletion of unused databases after 3, 6, 12 months depending on record count, new options for GeoJSon export, adjsut Publish menu, Favourites section for saved searches in Explore menu, CSV export sanisise within-field line breaks, add Ruleset to standard filter builder, months in Persian, small improvements to search display, generating archive package SQL only (no HML), remote lookup on Heurist db allow View record for retrieved records, link to open semantic URIs for conepts, staggered layout when multiple windows on top of one another for better contextualisation, improved workflow for new record type creastion within record poitner field definition, improved new record title mask handling, make CSV export 'advanced' functions more accessible, handle creation of duplicate field name intutitively, create new record types from a CSV file 

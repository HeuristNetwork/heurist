...
// The contents of this file are overridden by ..../HEURIST/heuristConfigIni.php
// It will also be overwritten eveny time you upgrade Heurist and your changes will be lost.

// *** DO NOT CHANGE VERSION NUMBER, THIS IS SET BY THE HEURIST DEVELOPMENT TEAM ***

$version = "6.3.12"; // sets current program version number, determined by Heurist development lead
...
// 6.3.10 05 Dec 2022 Upgrade to PHP8, remove deleted fulltext indexes (php 5 bug), initial version Nakala lookup, rename base field option when renaming field in a record type, add sorting to ESTC external lookup, link to switch between alpha and standard version with bug report, download remote file referenced in a file field and convert to local file reference, upload files referenced in a text list of URLs, avoid duplication of local files on upload, handle more than 500 fields in a single record type (!), allow simple dividers with no labelling, show and distinguiish all dividers in field treea, allow set proxy server even for internal resources, auto-load of child records in StoryMap widget when a record is displayed in record view, add tabs for multiple record display in StoryMap widget, improvements to database and table purging script, manual reordering of terms in branches of the tree, markign fields as private/logged-in/public and per-record temporary hiding of a field (checkbox) in record edit, change name of Reperesentative image/thumbnail to Primary/preferred image (new DBs and wherever name and label not edited), improve query handling, regitering json IIIF manifests, link to Mirador on View record, deletion of all values option when a field is deleted from a reocrd type, 3D model visualisation with 3DHOP for very compressed nxz files and Online 3D Viewer for 'obj', '3ds', 'stl', 'ply', 'gltf', 'glb', 'off', '3dm', 'fbx', 'dae', 'wrl', '3mf', 'ifc', 'brep', 'step', 'iges', 'fcstd', 'bim', Nakala lookup / submit / multi-file submit and remove from Heurist database / submit entire database archive to Nakala, fix Nomisma loookup, accordion facets in facet search, option to sort field trees in field order or alphabetically, improve layout of Record view particularly image links, from data entry open all records for which a given field has been recorded, allow accordion headings within blocks, allow one-click addition of non-standard fields at the bottom of data entry to a record type

// 6.3.9  06 oct 2022 - fix crosstabs, image size change on gallery, Storymap reset control, show terms usage count in manage vocabularies, show field usage count in record strucute modification

...
function stylesheet_exists($style){
  ...
  if (file_exists("../stylesheets/".$style)){ 
   	return true;
  } else {  
   	return false;
  }
 	...
 	$res = mysql_query('select * from saved_searches where ss_id='.$_REQUEST['pub_id']);
  	...
  	if (mysql_num_rows($res) != 0) {
		...
		case 'genericxml': //no javascript parameter for genericxml
			header('Location: http://'.HOST.'/cocoon/heurist/' . $_REQUEST['pub_id']);
			break;
    		
		default:
		
			if (stylesheet_exists($style)){
	  			header('Location: http://'.HOST.'/cocoon/heurist/' . $_REQUEST['pub_id'] .(!$output ? '': '/'. $output). '/' . $style . $js);
	  		} else {

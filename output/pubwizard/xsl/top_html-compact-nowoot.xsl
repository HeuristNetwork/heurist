<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 last updated 10/09/2007 ms
  ...
  <xsl:template name="header">
    ...
    <tr>
      <td style="font-size: 11px;">
        <xsl:value-of select="reftype"/> ; <xsl:if test="modified !=''"> &#160; <span
  ...
  <!-- url  reftype only -->
  <xsl:template match="reference[reftype/@id=1]">
          ...
          <xsl:choose>
            <xsl:when
              ...
              <xsl:variable name="spanid">
                <xsl:value-of select="../reftype/@id"/>
  ...
  <xsl:template name="body-pointer">
    ...
    <xsl:for-each select="pointer">
      <!--act on the first in document order-->
      <xsl:if test="generate-id(.)=generate-id($pointer[@id=current()/@id][1])">
        <span class="reftype">
          <xsl:choose>
            <xsl:when test="@name !=''">
              <xsl:value-of select="@name"/>

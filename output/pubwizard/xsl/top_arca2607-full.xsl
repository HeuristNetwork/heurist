<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="/">
   ...
   <xsl:comment>
			...
			[name]ARCA 2607 Full[/name]
			<!-- match the name of the stylesheet-->
			[output]arca2607-full[/output]
		</xsl:comment>
    <!-- end including code -->
    <html>
      <head>
    ...
    <tr>
      <td>
        ...
        <xsl:if test="detail[@id='160']">
          ...
          <xsl:apply-templates select="pointer[@id=158]" mode="creator"/>
  ...
  <!-- =================  5: BOOK =============================== -->
  <xsl:template name="publications" match="reference[reftype/@id=5]">
    <tr>
      <td>
        <!-- if Title are missing, don't print -->
        <xsl:if test="detail[@id='160']">
          ...
          <xsl:if test="pointer[@id = 228]/pointer[@id = 229]"> &#160;<xsl:apply-templates
  ...
  <!-- =================  4: BOOK CHAPTER  =============================== -->
  <xsl:template name="book_chap" match="reference[reftype/@id=4]">
    <tr>
      <td>
        ...
        <xsl:if test="detail[@id='160']">
          ...
          <xsl:apply-templates select="pointer[@id=227]/pointer[@id=158]" mode="creator"/>(ed.)
            &#xa0;<xsl:value-of select="pointer[@id=227]/detail[@id = '159']"/>, <i>
              &#xa0;<xsl:value-of select="pointer[@id=227]/detail[@id = '160']"/>. </i>
          <!-- edition or version -->
          <xsl:if test="pointer/detail[@id = '176']"> &#xa0;<xsl:value-of
              select="pointer/detail[@id = '176']"/>, </xsl:if>
          <!-- Volume -->
          <xsl:if test="pointer/detail[@id = '184']"> &#xa0;<xsl:value-of
              select="pointer/detail[@id = '184']"/>, </xsl:if>
          <!-- Publisher name, Place published -->
          <xsl:if test="pointer[@id=227]/pointer[@id = 228]"> &#xa0;<xsl:apply-templates
              select="pointer[@id=227]/pointer[@id = 228]"/>, </xsl:if>
          <!-- Pages -->
  ...
  <!-- =================  1: HYPERLINK  =============================== -->
  <xsl:template name="hyperlink" match="reference[reftype/@id=1]">
    <tr>
      <td>
        ...
        <xsl:if test="detail[@id='160'] and url[.!='']">
          ...
          <xsl:if test="pointer[@id = '158']">
            &#xa0;<xsl:apply-templates select="pointer[@id=158]" mode="creator"/></xsl:if>
          <!-- abstract -->
  ...
  <xsl:template name="journal" match="reference[reftype/@id=29 or reftype/@id=68 or reftype/@id=69]">
    <tr>
      <td>
    ...
    <tr>
      <td>
        <xsl:if test="detail[@id='160']">
          <!-- creator -->
          <xsl:apply-templates select="pointer[@id=158]" mode="creator"/>
          <xsl:if test="pointer[@id=238]/detail[@id=159]">
            <!-- year (from journal volume reference) --> &#160;<xsl:value-of
              select="pointer[@id=238]/detail[@id=159]"/>, </xsl:if>
          <!-- title --> &#160;<xsl:value-of select="detail[@id=160]"/>. <!--from conference -->
          <xsl:if test="pointer[@id=217]/detail[@id=160] and pointer[@id=217]/detail[@id=181]">
            &#xa0;In: <i>
              <xsl:value-of select="pointer[@id=217]/detail[@id=160]"/>
            </i>, <!--location, pages --> &#xa0;<xsl:value-of
              select="pointer[@id=217]/detail[@id=181]"/>. </xsl:if>
    ...
    <tr>
      <td>
        ...
        <xsl:if test="detail[@id=159]">
          ...
          <xsl:choose>
            <xsl:when test="pointer[@id=158]"> &#xa0;<xsl:value-of select="detail[@id=159]"/>, </xsl:when>
            <xsl:otherwise>
        ...
        <xsl:if test="pointer[@id=217]/detail[@id=160]">
          <!--conference reported : --> &#xa0;<xsl:apply-templates
            select="pointer[@id=217]/detail[@id=160]"/>. </xsl:if>
        <xsl:if test="pointer[@id=217]/detail[@id=159]"> &#xa0;<xsl:apply-templates
            select="pointer[@id=217]/detail[@id=159]"/>, </xsl:if>
        <!-- location -->
        <xsl:if test="pointer[@id=217]/detail[@id=181]"> &#xa0;<xsl:value-of
            select="pointer[@id=217]/detail[@id=181]"/>
        </xsl:if>
        <xsl:if test="detail[@id=187]">
          &#160;<xsl:value-of select="detail[@id=187]/@type"/>: <xsl:value-of select="detail[@id=187]"/>.
        </xsl:if>
  ...
  <xsl:template name="article" match="reference[reftype/@id=9 or reftype/@id=10]">
    ...
    <tr>
      <td>
        <xsl:if test="detail[@id='160']">
          ...
          <xsl:if test="pointer[@id=237]/detail[@id=166]/year"> &#xa0;<xsl:value-of
              select="pointer[@id=237]/detail[@id=166]/year"/>, </xsl:if>
          <!-- title --> &#xa0;<xsl:value-of select="detail[@id=160]"/>. <!--newspaper title -->
          <xsl:if test="pointer[@id=237]/pointer[@id=242] "> &#xa0; <i>
              <xsl:apply-templates select="pointer[@id=237]/pointer[@id=242]"/>, </i>
            <xsl:if test="pointer[@id=237]/detail[@id=166]">
              <!-- date --> &#xa0;<xsl:apply-templates mode="date"
                select="pointer[@id=237]/detail[@id=166]"/>. </xsl:if>
          </xsl:if>
          <!-- pages-->
          <xsl:if test="detail[@id=164] and detail[@id=165]"> &#xa0;<xsl:value-of
              select="detail[@id=164]"/>-<xsl:value-of select="detail[@id=165]"/>
    ...
    <tr>
      <td>
        <xsl:choose>
          ...
          <xsl:otherwise>
            <!-- title (from jounal reference) -->
            <xsl:value-of select="pointer[@id=226]/detail[@id=160]"/>.&#xa0; </xsl:otherwise>
        </xsl:choose>
  ...
  <!-- =================  30: PUBLISHER  =============================== -->
  <xsl:template name="publisher" match="reference[reftype/@id=30 ] ">
  ...
  <!-- =================  44: PUBLICATION SERIES  =============================== -->
  <xsl:template name="pub_series" match="reference[reftype/@id=44 ] ">
    <xsl:choose>
      ...
      <xsl:when
        ...
        <tr>
          <td>
            ...
            <xsl:if test="pointer[@id=229]/detail[@id=160]">
              <xsl:choose>
                <xsl:when test="pointer[@id=229]/detail[@id=172]"> &#160;<xsl:value-of
                    select="pointer[@id=229]/detail[@id=160]"/>: <xsl:value-of
                    select="pointer[@id=229]/detail[@id=172]"/>
                </xsl:when>
                <xsl:otherwise> &#160;<xsl:value-of select="pointer[@id=229]/detail[@id=160]"/>
                </xsl:otherwise>
              </xsl:choose>
            </xsl:if>
            <xsl:if test="detail[@id=187]">
  ...
  <xsl:template name="jour_article" match="reference[reftype/@id=3 or reftype/@id=10 ]">
    <tr>
      <td>
        <!-- creator -->
        <xsl:apply-templates select="pointer[@id=158]" mode="creator"/>
        ...
        <xsl:if test="pointer[@id=225]/detail[@id=159]"> &#160;<xsl:value-of
            select="pointer[@id=225]/detail[@id=159]"/>, </xsl:if>
        <!-- title --> &#160;<xsl:value-of select="detail[@id=160]"/>.&#xa0; <!-- journal (from journal volume reference) -->
        <xsl:if test="pointer[@id=225]/pointer[@id=226]">
          <i>
            <xsl:choose>
              <xsl:when test="pointer[@id=225]/detail[@id=184]">
                <xsl:apply-templates select="pointer[@id=225]/pointer[@id=226]"/>, </xsl:when>
              <xsl:otherwise>
                <xsl:apply-templates select="pointer[@id=225]/pointer[@id=226]"/>
              </xsl:otherwise>
            </xsl:choose>
          </i>
        </xsl:if>
        <!-- volume (from journal volume reference), formatting: if pages exist, put :  -->
        <xsl:if test="pointer[@id=225]/detail[@id=184]">
          <xsl:choose>
            <xsl:when test="detail[@id=164] and detail[@id=165]"> &#xa0;<xsl:value-of
                select="pointer[@id=225]/detail[@id=184]"/>: </xsl:when>
            <xsl:otherwise> &#xa0;<xsl:value-of select="pointer[@id=225]/detail[@id=184]"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:if>
        <!-- pages-->
  ...
  <xsl:template name="report" match="reference[reftype/@id=12 ]">
    <tr>
      <td>
        ...
        <i>
          <xsl:choose>
            <xsl:when test="pointer[@id=229]"> &#xa0;<xsl:value-of select="detail[@id=160]"/>, </xsl:when>
            <xsl:otherwise> &#xa0;<xsl:value-of select="detail[@id=160]"/>
            </xsl:otherwise>
          </xsl:choose>

        </i>
        <xsl:if test="pointer[@id=229]">
          <!--publisher reference --> to <xsl:apply-templates select="pointer[@id=229]"/>
        </xsl:if>
        <xsl:if test="detail[@id=187]">
          &#160;<xsl:value-of select="detail[@id=187]/@type"/>: <xsl:value-of select="detail[@id=187]"/>.
        </xsl:if>
  ...
  <xsl:template name="summary">
    <xsl:if test="detail[@id=303] ">
      <br></br>
      <xsl:copy-of select="detail[@id=303]"/>
    </xsl:if>

    <xsl:if test="detail[@id=191] ">
      <br></br>
      <xsl:copy-of select="detail[@id=191]"/>
    </xsl:if>
  </xsl:template>

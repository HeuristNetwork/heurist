...
/*
		...
		$("<div id='divZoomIn' title='Zoom In'><img src='"+RelBrowser.baseURL+"/images/plus.png'></img></div>")
			.click(function () {
				zoom(true);
			})
			.appendTo($div);
		$("<div  id='divZoomOut' title='Zoom Out'><img src='"+RelBrowser.baseURL+"/images/minus.png'></img></div>")
			.click(function () {
				zoom(false);
			})
			.appendTo($div);
	...
	* information window
	*/
	openInfoWindowHandler: function (_item) {
			var $preview, content;
			var item;
			...
			content = $(item).data("info");
			if (! content) {
				var recid = item.placemark.item.opts.recid;
				// grab the preview content
				if ( recid ) {
					if ($("#preview-" + recid + " .balloon-middle").length < 1) {

						var url =  RelBrowser.baseURL + "preview/" + recid;

						$preview = $("<div class='preview' id='preview-" + recid + "'/>").appendTo("#previews");
						$preview.load(url,function(){
								RelBrowser.Mapping.openInfoWindowHandler(item);
							});
					...
					$preview = $("#preview-" + recid + " .balloon-middle").clone();
					$preview.removeClass("balloon-middle").addClass("map-balloon");
					$(".balloon-content .clearfix", $preview).before(
						"<p><a href='" + recid + "'>more &raquo;</a></p>"
					);
					content = $preview.get(0);
				} else {
					content = item.dataset.getTitle();
				}
				$(item).data("info", content);
			}

			// open window
			if (item.getType() == "marker") {
				item.placemark.setInfoBubble(content);
				item.placemark.openBubble();
			} else {
				item.map.openBubble(item.getInfoPoint(), content); //????
			}
	...
	*
	*/
	initMap: function (map_div_id, mini) {
			...
			if (M.mapdata.focus) {
				...
				if (matches) {
					...
					if (points.length > 0) {
						bounds = new mxn.BoundingBox(points[0].lat, points[0].lon,
			...
			if (window["Timeline"]) { //there is Timeline object
				M.initMapAndTimeline(map_div_id, bounds, mini); //init timemap+timeline and fill with the custom data
			} else {
			...
			//tiled images
			var errors = M.addLayers(M.mapdata.layers, (M.mapdata.timemap.length>0)?0:1, bounds);
	...
	*/
	initMapAndTimeline: function (map_div_id, bounds, mini)
	{
			...
			/*ART 2013-02-04 tl_theme.mouseWheel = "default";//"zoom";
			...
			var __onDataLoaded = function (tm) {
				...
				if (eventSource.getCount() > 0) {
					...
					//there is only one band M.changeScale(1, zoomIndex + 3, tm);
					M.changeScale(0, zoomIndex, tm);
				}
				var band = tm.timeline.getBand(0);
				var ht = ((band.getEventPainter().getLayout().getTrackCount() + 2.5) * (1.3 + 0.2) ) + 'em';
				$("#timeline").height(ht);
				...
				}else if(M.mapdata.count_mapobjects>0){
	...
	// 					 mapCenter: myLatlng,
			...
			var editLinkIcon = "";
			/*"<div id='rec_edit_link' style='display:inline-block;' class='logged-in-only'><a href='"+
...
'</div></div>';
			...
			var customIcon = basePath + "images/16x16pngs/icon16-annotation.png";// "heuristicon.png"; //TODO!!!! - deault marker
			...
			M.tmap = TimeMap.init({
				...
				options: {
					mapZoom: 2,
					//ART 201302 useMarkerCluster: (M.mapdata.count_mapobjects<300),
					onlyTimeline: (M.mapdata.count_mapobjects<1),
					//ART 201302 infoTemplate: template,
					theme: customTheme,
					/*
					...
					*/
					eventIconPath: RelBrowser.baseURL + "js/timemap.js/2.0.1/images/",
					openInfoWindow: mini ? function () { return false; } : RelBrowser.Mapping.openInfoWindowHandler
				},
/*
		bandIntervals:[
...
*/
            ...
            var painter = M.tmap.timeline.getBand(0).getEventPainter().constructor;
            painter.prototype._showBubble = function(x,y,evt){}; //disable
	...
	*/
	addLayers: function (layers, zoom_mode, bounds) {

			var i, M = RelBrowser.Mapping,
				errors = "";

			if(M.map){
//ART v2				M.map.removeAllTileLayers();
			}else{
				return;
			}
...
function initMapping(map_div_id, mapdata, mini) {
	initMappingClass();
	// default to 100px per century
	RelBrowser.Mapping.initTimeZoomIndex = RelBrowser.Mapping.timeZoomSteps.length - 1;
	RelBrowser.Mapping.mapdata = mapdata;
	return RelBrowser.Mapping.initMap(map_div_id, mini);
}

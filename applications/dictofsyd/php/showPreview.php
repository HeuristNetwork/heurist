...
*/
function showPreview($rec_id, $is_cache=1){
	...
	if(is_numeric($rec_id)){
		...
		$filename_html = dirname(__FILE__)."/../preview/".$rec_id.".html";
		...
		if($is_cache==2 && file_exists($filename_html)){
			...
			$filename = dirname(__FILE__)."/../preview/".$rec_id.".xml";
			$xml = new DOMDocument();
			...
			if($xml){
				...
				$xsl = new DOMDocument;
				$xsl->load(dirname(__FILE__)."/../xsl/preview_standalone.xsl");
